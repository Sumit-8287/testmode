  ...
  login: async (adminId: string, password: string) => {
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        throw new Error(
          `Backend API not available. Status: ${response.status}`,
        );
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          error.error ||
            error.details ||
            `Failed to create test series (${response.status})`,
        );
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          error.error ||
            error.details ||
            `Failed to create subjective test (${response.status})`,
        );
      }
      const result = await response.json();
...
export const packagesAPI = {
  ...
  update: async (id: string, data: any) => {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.error || errorData.details || "Failed to update package",
      );
    }
    return response.json();
  },
...
export const chatsAPI = {
  ...
  getChats: async (studentId?: string) => {
    const url = studentId
      ? `${API_BASE_URL}/chats?studentId=${studentId}`
      : `${API_BASE_URL}/chats`;
    const response = await fetch(url);
...
export const referralsAdminAPI = {
  ...
  updateStatus: async (
    ...
    const response = await fetch(
      `${API_BASE_URL}/admin/referrals/update-status`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
...
export const splashScreenAPI = {
  ...
  update: async (data: any) => {
    ...
    if (!response.ok)
      throw new Error("Failed to update splash screen settings");
    return response.json();
  },
};
