...
app.get("/api/download/apk", (req, res) => {
  try {
    ...
    if (apkPath && fs.existsSync(apkPath)) {
      ...
      res
        .status(404)
        .json({ error: "APK file not found. Please upload via admin panel." });
    }
  } catch (error) {
...
app.put("/api/courses", async (req, res) => {
  try {
    ...
    for (const update of updates) {
      ...
      await db.collection("courses").updateOne(filter, {
...
app.put("/api/courses/:id/videos/:videoId", async (req, res) => {
  try {
    ...
    if (course) {
      query.courseId = {
        $in: [course.id, course._id.toString(), req.params.id],
      };
    }

    const { _id, ...updateData } = req.body;
    const result = await db.collection("videos").updateOne(query, {
      $set: { ...updateData, updatedAt: new Date().toISOString() },
    });
...
app.get("/api/courses/:id/notes", async (req, res) => {
  try {
    ...
    const query = {
      courseId: {
        $in: [course.id, course._id.toString(), req.params.id].filter(Boolean),
      },
    };
    ...
    notes.forEach((n) => {
      if (
        !allNotes.find(
          (p) => p.id === n.id || p._id.toString() === n._id.toString(),
        )
      )
        allNotes.push(n);
    });
...
app.put("/api/courses/:id/notes/:noteId", async (req, res) => {
  try {
    ...
    let result = await db.collection("pdfs").updateOne(query, {
      $set: { ...updateData, updatedAt: new Date().toISOString() },
    });
    if (result.matchedCount === 0) {
      result = await db.collection("notes").updateOne(query, {
        $set: { ...updateData, updatedAt: new Date().toISOString() },
      });
    }
    if (result.matchedCount === 0)
...
app.get("/api/courses/:id/tests", async (req, res) => {
  try {
    ...
    const query = {
      courseId: {
        $in: [req.params.id, alternativeId, mongoId].filter(Boolean),
      },
      status: "active",
    };
    ...
    console.log(
      `GET /api/courses/${req.params.id}/tests - Found ${tests.length} tests (strictly for this course)`,
    );
    res.json(tests);
  } catch (error) {
...
app.put("/api/courses/:id", async (req, res) => {
  try {
    console.log(
      `PUT /api/courses/${req.params.id} - Body size: ${JSON.stringify(req.body).length} bytes`,
    );
    const { _id, ...updateData } = req.body;
    ...
    if (!record) {
      console.warn(
        `[UPDATE FAILED] No record found in any collection for: ${id}`,
      );
      return res.status(404).json({ error: "Package/Course not found in DB" });
    }

    console.log(
      `[TARGET MATCH] Collection: ${targetCollection}, DB _id: ${record._id}`,
    );
    ...
    try {
      ...
      const students = jsonData
        .map((row, idx) => ({
          ...
          registrationType:
            row.registrationType || row["Registration Type"] || "regular",
      ...
      res
        .status(500)
        .json({ error: "Failed to process Excel file: " + error.message });
    }
  },
);
...
app.delete("/api/buyers", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Deleted ${result.deletedCount} buyers`,
    });
  } catch (error) {
...
app.post("/api/buyers/bulk", async (req, res) => {
  try {
    ...
    if (!Array.isArray(buyers) || buyers.length === 0)
      return res.status(400).json({ error: "No buyers provided" });
...
app.delete("/api/coupons", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Deleted ${result.deletedCount} coupons`,
    });
  } catch (error) {
...
app.post("/api/admin/login", async (req, res) => {
  try {
    ...
    console.log(
      `[LOGIN ATTEMPT] AdminID: ${adminId}, Time: ${new Date().toISOString()}`,
    );
  ...
  try {
    ...
    if (type === "Weekly") {
      ...
      while (curr <= end) {
        ...
        dailyStats[key] = {
          name: `Week ${Math.ceil(curr.getDate() / 7)}`,
          sales: 0,
          revenue: 0,
          signups: 0,
          _s: curr.getTime(),
          _e: curr.getTime() + 7 * 24 * 60 * 60 * 1000,
        };
      ...
      while (curr <= end) {
        ...
        dailyStats[key] = {
          name: curr.toLocaleDateString("en-GB", {
            day: "2-digit",
            month: "short",
          }),
          sales: 0,
    ...
    purchases.forEach((p) => {
      ...
      let key;
      if (type === "Weekly")
        key = Object.keys(dailyStats).find(
          (k) =>
            d.getTime() >= dailyStats[k]._s && d.getTime() < dailyStats[k]._e,
        );
...
app.get("/api/institute", async (req, res) => {
  try {
    ...
    res.json(
      settings || { name: "", email: "", phone: "", address: "", logo: "" },
    );
  } catch (error) {
...
app.put("/api/questions/update-all", async (req, res) => {
  try {
    ...
    for (const update of updates) {
      const { id, _id, ...data } = update;
      if (id)
        await db.collection("questions").updateOne({ id }, { $set: data });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (questions.length === 0)
        return res.status(400).json({ error: "No valid question data found" });
      const result = await db.collection("questions").insertMany(questions);
      res.status(201).json({ success: true, inserted: result.insertedCount });
    } catch (error) {
      res
        .status(500)
        .json({ error: "Failed to process Excel: " + error.message });
    }
  },
);
...
app.get("/api/tests/:id", async (req, res) => {
  try {
    ...
    const separateQuestions = await db
      .collection("questions")
      .find({ testId: req.params.id })
      .toArray();
    const embeddedQuestions = test.questions || [];
    const questions =
      separateQuestions.length > 0 ? separateQuestions : embeddedQuestions;
  ...
  try {
    ...
    if (!test) {
      try {
        test = await db
          .collection("tests")
          .findOne({ _id: new mongoose.Types.ObjectId(req.params.testId) });
      } catch (e) {}
    }
    if (!test) {
      return res.status(404).json({ error: "Test not found" });
    ...
    const separateQuestions = await db
      .collection("questions")
      .find({ testId: req.params.testId })
      .toArray();
    const questions =
      separateQuestions.length > 0 ? separateQuestions : test.questions || [];
...
app.post("/api/tests", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create test", details: error.message });
  }
});
...
app.delete("/api/tests/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete test", details: error.message });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      const newQuestions = jsonData.map((row, i) => ({
        ...
        negativeMarks:
          parseFloat(row.negativeMarks || row["Negative Marks"]) || 0,
      }));
      const allQuestions = [...existingQuestions, ...newQuestions];
      if (test) {
        await db
          .collection("tests")
          .updateOne({ _id: test._id }, { $set: { questions: allQuestions } });
      }
      res.json({
        success: true,
...
app.post("/api/test-series", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create test series", details: error.message });
  }
});
...
app.put("/api/test-series/:id", async (req, res) => {
  try {
    console.log(
      "PUT /api/test-series/:id - Updating series:",
      req.params.id,
      req.body,
    );
    const { _id, ...updateData } = req.body;
    ...
    res
      .status(500)
      .json({ error: "Failed to update test series", details: error.message });
  }
});
...
app.delete("/api/test-series/:id", async (req, res) => {
  try {
    console.log(
      "DELETE /api/test-series/:id - Deleting series:",
      req.params.id,
    );
    const result = await db
      .collection("testSeries")
    ...
    res
      .status(500)
      .json({ error: "Failed to delete test series", details: error.message });
  }
});
...
app.delete("/api/test-series", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Deleted ${result.deletedCount} test series`,
    });
  } catch (error) {
...
app.post("/api/test-series/bulk", async (req, res) => {
  try {
    ...
    if (!Array.isArray(series) || series.length === 0)
      return res.status(400).json({ error: "No series provided" });
...
app.put("/api/test-series/update-all", async (req, res) => {
  try {
    ...
    for (const update of updates) {
      const { id, _id, ...data } = update;
      if (id)
        await db.collection("testSeries").updateOne({ id }, { $set: data });
    }
...
app.post("/api/subjective-tests", async (req, res) => {
  try {
    ...
    console.log(
      "Subjective test created successfully with ID:",
      result.insertedId,
    );
    res.status(201).json({ _id: result.insertedId, ...req.body });
  } catch (error) {
    ...
    res.status(500).json({
      error: "Failed to create subjective test",
...
app.put("/api/subjective-tests/:id", async (req, res) => {
  try {
    console.log(
      "PUT /api/subjective-tests/:id - Updating test:",
      req.params.id,
      req.body,
    );
    const { _id, ...updateData } = req.body;
    const result = await db
      .collection("subjectiveTests")
      .updateOne({ id: req.params.id }, { $set: updateData });
    if (result.matchedCount === 0)
      return res.status(404).json({ error: "Test not found" });
    ...
    res.status(500).json({
      error: "Failed to update subjective test",
      details: error.message,
    });
  }
});
...
app.delete("/api/subjective-tests/:id", async (req, res) => {
  try {
    console.log(
      "DELETE /api/subjective-tests/:id - Deleting test:",
      req.params.id,
    );
    const result = await db
      .collection("subjectiveTests")
    ...
    res.status(500).json({
      error: "Failed to delete subjective test",
      details: error.message,
    });
  }
});
...
app.delete("/api/subjective-tests", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Deleted ${result.deletedCount} subjective tests`,
    });
  } catch (error) {
...
app.put("/api/subjective-tests/update-all", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Updated ${updates.length} subjective tests`,
    });
  } catch (error) {
...
app.delete("/api/courses/:courseId/videos/:videoId", async (req, res) => {
  try {
    const result = await db
      .collection("videos")
      .deleteOne({ id: req.params.videoId, courseId: req.params.courseId });
    if (result.deletedCount === 0)
      return res.status(404).json({ error: "Video not found" });
    res.json({ success: true, message: "Video deleted" });
  } catch (error) {
...
app.delete("/api/videos", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Deleted ${result.deletedCount} videos`,
    });
  } catch (error) {
...
app.put("/api/live-videos/:id", async (req, res) => {
  try {
    ...
    const result = await db
      ...
      .updateOne({ id: req.params.id }, { $set: updateData });
    if (result.matchedCount === 0)
      return res.status(404).json({ error: "Live video not found" });
...
app.delete("/api/courses/:courseId/notes/:noteId", async (req, res) => {
  try {
    const result = await db
      .collection("pdfs")
      .deleteOne({ id: req.params.noteId, courseId: req.params.courseId });
    if (result.deletedCount === 0)
      return res.status(404).json({ error: "Note not found" });
    res.json({ success: true, message: "Note deleted" });
  } catch (error) {
  ...
  try {
    ...
    const query = {
      $or: [
        { id: testId },
        {
          _id: mongoose.Types.ObjectId.isValid(testId)
    ...
    if (course) {
      query.courseId = {
        $in: [course.id, course._id.toString(), req.params.courseId],
      };
    }

    const { _id, ...updateData } = req.body;
    const result = await db.collection("tests").updateOne(query, {
      $set: { ...updateData, updatedAt: new Date().toISOString() },
    });
    if (result.matchedCount === 0)
      return res.status(404).json({ error: "Test not found" });
...
app.put("/api/packages/:id", async (req, res) => {
  try {
    console.log(
      `PUT /api/packages/${req.params.id} - Body size: ${JSON.stringify(req.body).length} bytes`,
    );
    const { _id, ...updateData } = req.body;
    const id = req.params.id;
    ...
    if (!record) {
      console.warn(
        `[UPDATE FAILED] Package/Course not found in DB for id: ${id}`,
      );
      return res.status(404).json({ error: "Package/Course not found in DB" });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to update package", details: error.message });
  }
});
...
const otpLimiter = rateLimit({
  ...
  message: {
    error:
      "Too many OTP requests from this IP, please try again after a minute",
  },
});
...
app.post("/api/otp/send", otpLimiter, async (req, res) => {
  try {
    ...
    if (
      !smsResponse.ok ||
      (smsResult && smsResult.toLowerCase().includes("error"))
    ) {
...
app.post("/api/otp/verify", async (req, res) => {
  try {
    ...
    if (!stored) {
      return res
        .status(400)
        .json({ error: "OTP expired or not sent. Please request a new OTP." });
    }
...
app.post("/api/students/login", async (req, res) => {
  try {
    ...
    const student = await db
      .collection("students")
      .findOne({ $or: [{ phone }, { phone: cleanPhone }] });
...
app.post("/api/students/session/validate", async (req, res) => {
  try {
    ...
    if (!studentId || !sessionToken) {
      return res
        .status(400)
        .json({ error: "studentId and sessionToken are required" });
    }
    ...
    if (
      ...
      return res
        .status(401)
        .json({ valid: false, error: "Session invalid or expired" });
    }
...
app.get("/api/students/:id/courses", async (req, res) => {
  try {
    ...
    const courses = await db
      .collection("courses")
      .find({
        $or: [{ id: { $in: enrolledCourseIds } }, { _id: { $in: objectIds } }],
      })
      .toArray();
...
app.post("/api/students/:id/enroll", async (req, res) => {
  try {
    ...
    if (
      enrolledCourses.includes(canonicalId) ||
    ...
    await db
      .collection("courses")
      .updateOne({ _id: course._id }, { $inc: { studentsEnrolled: 1 } });
...
app.get("/api/students/:id/enrolled/:courseId", async (req, res) => {
  try {
    ...
    const enrolledCourses = student.enrolledCourses || [];
    const isEnrolled =
      enrolledCourses.includes(course.id || req.params.courseId) ||
      enrolledCourses.includes(course._id.toString());
  ...
  try {
    ...
    const result = await db.collection("downloads").deleteOne({
      $or: [
        ...
        {
          _id:
            global.ObjectId && global.ObjectId.isValid(downloadId)
              ? new global.ObjectId(downloadId)
              : null,
        },
      ].filter((v) => v.id || v._id),
      studentId: req.params.id,
...
app.delete("/api/courses/:courseId/live-classes/:id", async (req, res) => {
  try {
    const result = await db
      .collection("liveClasses")
      .deleteOne({ id: req.params.id, courseId: req.params.courseId });
    if (result.deletedCount === 0)
      return res.status(404).json({ error: "Live class not found" });
    res.json({ success: true, message: "Live class deleted" });
  } catch (error) {
...
app.delete("/api/categories/:id", async (req, res) => {
  try {
    ...
    await db
      ...
      .deleteMany({ categoryId: req.params.id });
...
app.post("/api/categories/seed", async (req, res) => {
  try {
    ...
    if (count > 0)
      return res.json({ message: "Categories already seeded", count });
    ...
    const defaultSubcategories = [
      {
        categoryId: "neet",
        id: "neet_class-11_recorded-batch",
        title: "Class 11th - Recorded Batch",
        parentPath: "Class 11th",
        icon: "video_library",
        color: "bg-blue-500",
        order: 1,
        isActive: true,
      ...
      {
        categoryId: "neet",
        id: "neet_class-11_live-classroom",
        title: "Class 11th - Live Classroom",
        parentPath: "Class 11th",
      ...
      {
        categoryId: "neet",
        id: "neet_class-11_crash-course",
        title: "Class 11th - Crash Course",
        parentPath: "Class 11th",
        icon: "speed",
        color: "bg-orange-500",
        order: 3,
      ...
      {
        categoryId: "neet",
        id: "neet_class-11_mock-test",
        title: "Class 11th - Mock Test",
        parentPath: "Class 11th",
        icon: "quiz",
        color: "bg-green-500",
        order: 4,
      ...
      {
        categoryId: "neet",
        id: "neet_class-12_recorded-batch",
        title: "Class 12th - Recorded Batch",
        parentPath: "Class 12th",
        icon: "video_library",
        color: "bg-blue-500",
        order: 5,
        isActive: true,
      ...
      {
        categoryId: "neet",
        id: "neet_class-12_live-classroom",
        title: "Class 12th - Live Classroom",
        parentPath: "Class 12th",
        icon: "cast_for_education",
        color: "bg-red-500",
        order: 6,
      ...
      {
        categoryId: "neet",
        id: "neet_class-12_crash-course",
        title: "Class 12th - Crash Course",
        parentPath: "Class 12th",
        icon: "speed",
        color: "bg-orange-500",
        order: 7,
      ...
      {
        categoryId: "neet",
        id: "neet_class-12_mock-test",
        title: "Class 12th - Mock Test",
        parentPath: "Class 12th",
        icon: "quiz",
        color: "bg-green-500",
        order: 8,
      ...
      {
        categoryId: "neet",
        id: "neet_neet-exam_recorded-batch",
        title: "NEET Exams - Recorded Batch",
        parentPath: "NEET Exams",
        icon: "video_library",
        color: "bg-blue-500",
        order: 9,
        isActive: true,
      ...
      {
        categoryId: "neet",
        id: "neet_neet-exam_live-classroom",
        title: "NEET Exams - Live Classroom",
        parentPath: "NEET Exams",
        icon: "cast_for_education",
        color: "bg-red-500",
        order: 10,
      ...
      {
        categoryId: "neet",
        id: "neet_neet-exam_crash-course",
        title: "NEET Exams - Crash Course",
        parentPath: "NEET Exams",
        icon: "speed",
        color: "bg-orange-500",
        order: 11,
      ...
      {
        categoryId: "neet",
        id: "neet_neet-exam_mock-test",
        title: "NEET Exams - Mock Test",
        parentPath: "NEET Exams",
        icon: "quiz",
        color: "bg-green-500",
        order: 12,
      ...
      {
        categoryId: "nursing",
        id: "nursing_bsc-cet-entrance",
        title: "BSC CET Entrance",
        parentPath: "",
        icon: "school",
        color: "bg-blue-500",
        description: "Nursing & Paramedical entrance",
        order: 1,
        isActive: true,
      },
      {
        categoryId: "nursing",
        id: "nursing_nursing-officer",
        title: "Nursing Officer",
        parentPath: "",
        icon: "medical_services",
        color: "bg-red-500",
        description: "Govt Job Coaching",
        order: 2,
        isActive: true,
      },
      {
      ...
      },
      {
        ...
        icon: "medication",
        color: "bg-green-500",
        description: "1st, 2nd, 3rd Year Syllabus",
        order: 4,
        isActive: true,
      },
      {
        categoryId: "nursing",
        id: "nursing_bsc-nursing",
      ...
      },
      {
        ...
        icon: "auto_stories",
        color: "bg-indigo-500",
        description: "Study material & notes",
        order: 6,
        isActive: true,
      },
      {
        categoryId: "nursing",
        id: "nursing_mock-test",
        title: "Mock Test",
        parentPath: "",
        icon: "quiz",
        color: "bg-teal-500",
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-9_english",
        title: "CBSE Class 9th - English",
        parentPath: "CBSE Board > Class 9th",
        icon: "translate",
        color: "bg-blue-500",
        order: 1,
        isActive: true,
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-9_hindi",
        title: "CBSE Class 9th - Hindi",
        parentPath: "CBSE Board > Class 9th",
        icon: "language",
        color: "bg-orange-500",
        order: 2,
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-9_social-studies",
        title: "CBSE Class 9th - Social Studies",
        parentPath: "CBSE Board > Class 9th",
        icon: "public",
        color: "bg-green-500",
        order: 3,
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-9_science",
        title: "CBSE Class 9th - Science & Tech",
        parentPath: "CBSE Board > Class 9th",
        icon: "science",
        color: "bg-purple-500",
        order: 4,
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-9_maths",
        title: "CBSE Class 9th - Mathematics",
        parentPath: "CBSE Board > Class 9th",
        icon: "calculate",
        color: "bg-red-500",
        order: 5,
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-10_english",
        title: "CBSE Class 10th - English",
        parentPath: "CBSE Board > Class 10th",
        icon: "translate",
        color: "bg-blue-500",
        order: 6,
        isActive: true,
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-10_hindi",
        title: "CBSE Class 10th - Hindi",
        parentPath: "CBSE Board > Class 10th",
        icon: "language",
        color: "bg-orange-500",
        order: 7,
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-10_social-studies",
        title: "CBSE Class 10th - Social Studies",
        parentPath: "CBSE Board > Class 10th",
        icon: "public",
        color: "bg-green-500",
        order: 8,
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-10_science",
        title: "CBSE Class 10th - Science & Tech",
        parentPath: "CBSE Board > Class 10th",
        icon: "science",
        color: "bg-purple-500",
        order: 9,
      ...
      {
        categoryId: "general",
        id: "general_cbse_class-10_maths",
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-9_english",
        title: "HBSE Class 9th - English",
        parentPath: "HBSE Board > Class 9th",
        icon: "translate",
        color: "bg-blue-500",
        order: 11,
        isActive: true,
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-9_hindi",
        title: "HBSE Class 9th - Hindi",
        parentPath: "HBSE Board > Class 9th",
        icon: "language",
        color: "bg-orange-500",
        order: 12,
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-9_social-studies",
        title: "HBSE Class 9th - Social Studies",
        parentPath: "HBSE Board > Class 9th",
        icon: "public",
        color: "bg-green-500",
        order: 13,
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-9_science",
        title: "HBSE Class 9th - Science & Tech",
        parentPath: "HBSE Board > Class 9th",
        icon: "science",
        color: "bg-purple-500",
        order: 14,
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-9_maths",
        title: "HBSE Class 9th - Mathematics",
        parentPath: "HBSE Board > Class 9th",
        icon: "calculate",
        color: "bg-red-500",
        order: 15,
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-10_english",
        title: "HBSE Class 10th - English",
        parentPath: "HBSE Board > Class 10th",
        icon: "translate",
        color: "bg-blue-500",
        order: 16,
        isActive: true,
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-10_hindi",
        title: "HBSE Class 10th - Hindi",
        parentPath: "HBSE Board > Class 10th",
        icon: "language",
        color: "bg-orange-500",
        order: 17,
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-10_social-studies",
        title: "HBSE Class 10th - Social Studies",
        parentPath: "HBSE Board > Class 10th",
        icon: "public",
        color: "bg-green-500",
        order: 18,
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-10_science",
        title: "HBSE Class 10th - Science & Tech",
        parentPath: "HBSE Board > Class 10th",
        icon: "science",
        color: "bg-purple-500",
        order: 19,
      ...
      {
        categoryId: "general",
        id: "general_hbse_class-10_maths",
        title: "HBSE Class 10th - Mathematics",
        parentPath: "HBSE Board > Class 10th",
        icon: "calculate",
        color: "bg-red-500",
        order: 20,
...
app.post("/api/referrals/generate", async (req, res) => {
  try {
    ...
    await db
      .collection("students")
      .updateOne({ id: studentId }, { $set: { referralCode: code } });
...
app.get("/api/referrals/:studentId", async (req, res) => {
  try {
    ...
    res.json({
      ...
      totalReferrals: referral.referredStudents
        ? referral.referredStudents.length
        : 0,
...
app.get("/api/referrals/:studentId/history", async (req, res) => {
  try {
    ...
    if (
      !referral ||
      !referral.referredStudents ||
      referral.referredStudents.length === 0
    ) {
      return res.json([]);
    }
...
app.post("/api/referrals/apply", async (req, res) => {
  try {
    ...
    if (!referralCode || !newStudentId) {
      return res
        .status(400)
        .json({ error: "Referral code and new student ID are required" });
    }
    ...
    const alreadyReferred =
      referral.referredStudents &&
      referral.referredStudents.some((r) => r.studentId === newStudentId);
    if (alreadyReferred) {
      return res
        .status(400)
        .json({ error: "This student has already been referred" });
    }
    ...
    const earning =
      settings.commissionType === "percentage"
        ...
        : settings.commissionValue || 50;

    const newStudent = await db
      .collection("students")
      .findOne({ id: newStudentId });
    const referredEntry = {
    ...
    await db
      .collection("students")
      .updateOne({ id: newStudentId }, { $set: { referredBy: referralCode } });
  ...
  try {
    ...
    if (!referralCode || !referredStudentId || !status) {
      return res.status(400).json({
        error: "referralCode, referredStudentId, and status are required",
      });
    }
    ...
    const entry = referral.referredStudents?.find(
      (rs) => rs.studentId === referredStudentId,
    );
  ...
  try {
    ...
    if (!keyId || !keySecret) {
      ...
      return res.status(500).json({
        error:
          "Razorpay credentials not configured correctly. Please check .env or Admin Settings.",
      });
    }

    console.log(
      `Razorpay Mode: ${keyId.startsWith("rzp_live") ? "LIVE (Real Money)" : "TEST (Sandbox)"}`,
    );
    ...
    if (coursePrice <= 0) {
      return res
        .status(400)
        .json({ error: "This course is free, no payment needed" });
    }
...
app.post("/api/razorpay/verify", async (req, res) => {
  try {
    ...
    if (paymentData.amount < expectedAmount) {
      ...
      return res
        .status(400)
        .json({ error: "Payment amount does not match course price" });
    }
    ...
    if (referralCode) {
      ...
      if (referral) {
        const settings = (await db
          .collection("referralSettings")
          .findOne({})) || { commissionType: "fixed", commissionValue: 50 };
        let earning = settings.commissionValue || 50;
        if (settings.commissionType === "percentage") {
          earning = Math.round(
            (purchase.amount * settings.commissionValue) / 100,
          );
        }
        const alreadyReferred =
          referral.referredStudents &&
          referral.referredStudents.some((r) => r.studentId === studentId);
        if (!alreadyReferred) {
          await db.collection("referrals").updateOne(
            { referralCode },
            {
...
app.post("/api/purchases", async (req, res) => {
  try {
    const { studentId, courseId, amount, paymentMethod, referralCode } =
      req.body;
...
app.get("/api/chats/unread/admin", async (req, res) => {
  try {
    const result = await db
      .collection("chats")
      .aggregate([{ $group: { _id: null, total: { $sum: "$unreadAdmin" } } }])
      .toArray();
...
app.post("/api/chats/start", async (req, res) => {
  try {
    ...
    const chatId =
      "chat_" + Date.now() + "_" + Math.random().toString(36).substr(2, 6);
    const chat = {
      id: chatId,
      studentId,
...
app.post("/api/chats/:chatId/messages", async (req, res) => {
  try {
    ...
    const unreadField =
      senderType === "student" ? "unreadAdmin" : "unreadStudent";
...
app.put("/api/chats/:chatId/read", async (req, res) => {
  try {
    ...
    const unreadField =
      readerType === "admin" ? "unreadAdmin" : "unreadStudent";
    await db
      .collection("chats")
      .updateOne({ id: req.params.chatId }, { $set: { [unreadField]: 0 } });
    res.json({ success: true });
  } catch (error) {
